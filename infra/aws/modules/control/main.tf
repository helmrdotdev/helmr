locals {
  ...
  control_secrets = {
    ...
    HELMR_WORKER_POOL_REGISTRATION_TOKEN = aws_secretsmanager_secret.worker_pool_registration_token.arn
...
resource "random_password" "worker_pool_registration_token" {
  length  = 64
  special = false
}

resource "random_password" "worker_token_signing_key" {
  length  = 64
  special = false
}
...
resource "random_bytes" "checkpoint_encryption_key" {
  length = 32
}
...
resource "aws_cloudfront_distribution" "control" {
  ...
  origin {
    domain_name = aws_lb.control.dns_name
...
resource "aws_ecs_service" "control" {
  ...
  lifecycle {
    precondition {
      condition     = var.certificate_arn != null || var.allow_insecure_http || var.enable_cloudfront
      error_message = "certificate_arn is required when create_control_service is true unless allow_insecure_http or enable_cloudfront is explicitly enabled."
    }
  }
}
...
resource "aws_secretsmanager_secret_version" "worker_token_signing_key" {
  secret_id     = aws_secretsmanager_secret.worker_token_signing_key.id
  secret_string = random_password.worker_token_signing_key.result
}
...
resource "aws_secretsmanager_secret_version" "secret_encryption_key" {
  ...
  secret_string = random_bytes.secret_encryption_key.base64
}
...
resource "aws_secretsmanager_secret_version" "checkpoint_encryption_key" {
  secret_id     = aws_secretsmanager_secret.checkpoint_encryption_key.id
  secret_string = random_bytes.checkpoint_encryption_key.base64
}

resource "aws_secretsmanager_secret" "worker_pool_registration_token" {
  name                    = "${local.name}/worker-pool-registration-token"
  kms_key_id              = aws_kms_key.helmr.arn
  recovery_window_in_days = var.secret_recovery_window_in_days
  tags                    = var.tags
}

resource "aws_secretsmanager_secret_version" "worker_pool_registration_token" {
  secret_id     = aws_secretsmanager_secret.worker_pool_registration_token.id
  secret_string = random_password.worker_pool_registration_token.result
}
