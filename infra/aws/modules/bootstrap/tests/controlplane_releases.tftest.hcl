...
override_resource {
  target = aws_kms_key.controlplane_releases
  values = {
...
run "controlplane_release_repository_is_durable_and_immutable" {
  ...
  assert {
    condition = (
      aws_ecr_repository.controlplane_releases.name == "helmr-test/controlplane-releases" &&
      aws_ecr_repository.controlplane_releases.image_tag_mutability == "IMMUTABLE" &&
      !aws_ecr_repository.controlplane_releases.force_delete &&
      aws_ecr_repository.controlplane_releases.image_scanning_configuration[0].scan_on_push &&
      aws_ecr_repository.controlplane_releases.encryption_configuration[0].encryption_type == "KMS" &&
      aws_ecr_repository.controlplane_releases.encryption_configuration[0].kms_key == aws_kms_key.controlplane_releases.arn
    )
    error_message = "Control Plane release images must live in one immutable, scan-enabled, KMS-encrypted, non-force-deletable repository."
  }

  assert {
    condition = (
      output.controlplane_release_repository_url == aws_ecr_repository.controlplane_releases.repository_url &&
      output.controlplane_release_repository_name == aws_ecr_repository.controlplane_releases.name &&
