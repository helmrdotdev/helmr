locals {
  ...
  sealed_boundary_arn = var.sealed_provider_definition == null ? null : try(var.sealed_provider_definition.boundary_policy_arn, null)
  use_external_boundary = var.sealed_provider_definition == null ? (
    var.external_permissions_boundary_arn != null
    ) : (
    local.sealed_boundary_arn != null
  )
  effective_external_boundary_arn = local.use_external_boundary ? (
    var.sealed_provider_definition == null ? var.external_permissions_boundary_arn : local.sealed_boundary_arn
  ) : null
}
...
moved {
  from = aws_iam_policy.worker_boundary
  to   = aws_iam_policy.worker_boundary[0]
}
  ...
  lifecycle {
    precondition {
      condition = (
        ...
        can(regex(":iam::${data.aws_caller_identity.current.account_id}:policy/", local.effective_external_boundary_arn))
      )
    ...
    precondition {
      condition = (
        ...
        var.external_permissions_boundary_arn == null ||
        !local.use_external_boundary ||
        var.external_permissions_boundary_arn == local.sealed_boundary_arn
      )
      error_message = "external_permissions_boundary_arn must match the sealed generation boundary or remain unset for retained pools."
    }
