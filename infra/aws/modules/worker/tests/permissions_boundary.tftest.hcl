...
run "legacy_retain" {
  ...
  variables {
    ...
    sealed_provider_definition = {
      user_data_base64                                = run.default_export.sealed_provider_definition.user_data_base64
      permission_policy_json                          = run.default_export.sealed_provider_definition.permission_policy_json
      boundary_policy_json                            = run.default_export.sealed_provider_definition.boundary_policy_json
      enable_ssm                                      = false
      launch_template_version                         = run.default_export.sealed_provider_definition.launch_template_version
  ...
  assert {
    ...
    error_message = "legacy sealed objects without boundary_policy_arn must remain module-managed"
  }
}
...
run "external_boundary_is_not_module_managed" {
  ...
  variables {
    external_permissions_boundary_arn = "arn:aws:iam::111122223333:policy/org-worker-boundary"
  }

  assert {
    condition = (
      length(aws_iam_policy.worker_boundary) == 0 &&
      aws_iam_role.worker.permissions_boundary == var.external_permissions_boundary_arn &&
      output.sealed_provider_definition.boundary_policy_arn == var.external_permissions_boundary_arn &&
...
run "external_boundary_rejects_foreign_account" {
  ...
  variables {
    external_permissions_boundary_arn = "arn:aws:iam::999999999999:policy/org-worker-boundary"
  }
...
run "retained_external_boundary_rejects_input_switch" {
  ...
  variables {
    external_permissions_boundary_arn = "arn:aws:iam::111122223333:policy/other-worker-boundary"
    sealed_provider_definition = {
