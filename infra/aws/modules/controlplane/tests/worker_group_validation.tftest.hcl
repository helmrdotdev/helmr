...
variables {
  ...
  worker_groups = [{
    ...
    instance_capacity = {
      ...
      build_cache_bytes = 0, artifact_cache_bytes = 0, vm_slots = 2, build_executors = 0
    }
  }]
}
...
run "nineteen_character_name_fits_aws_resource_limits" {
  command = plan

  variables {
    name                         = "helmr-1234567890123"
    vpc_id                       = "vpc-0123456789abcdef0"
    private_subnet_ids           = ["subnet-0123456789abcdef0", "subnet-1123456789abcdef0"]
    public_subnet_ids            = ["subnet-2123456789abcdef0", "subnet-3123456789abcdef0"]
    controlplane_image           = "example.invalid/helmr@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
  ...
  assert {
    condition = (
      length(aws_lb.controlplane.name) == 32 &&
      length(aws_lb_target_group.controlplane.name) == 32
    )
    error_message = "the longest accepted prefix must produce valid 32-character ALB and target-group names."
  }
}

run "twenty_character_name_is_rejected" {
  command = plan

  variables {
    name                         = "helmr-12345678901234"
    vpc_id                       = "vpc-0123456789abcdef0"
    private_subnet_ids           = ["subnet-0123456789abcdef0", "subnet-1123456789abcdef0"]
    public_subnet_ids            = ["subnet-2123456789abcdef0", "subnet-3123456789abcdef0"]
    controlplane_image           = "example.invalid/helmr@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
