...
locals {
  ...
  # The current definition wins if an operator accidentally repeats its exact
  # Pool key in the retained set. A changed immutable definition produces a new
  # key and therefore a second ASG/LT that can coexist at scale zero.
  tags = merge({
    Project     = "helmr"
    Application = "helmr"
