...
assert_contains "$sql_stdin" "runtime_checkpoints" "checkpoint evidence query should be present"
assert_contains "$sql_stdin" "checkpoint_artifact" "checkpoint artifact detail query should be present"
assert_contains "$sql_stdin" "checkpoint_artifact_summary" "checkpoint artifact aggregate query should be present"
assert_contains "$sql_stdin" "checkpoint_phase" "checkpoint creation phase query should be present"
assert_contains "$sql_stdin" "runtime_checkpoint_phases.duration_ms" "checkpoint creation phase duration should be reported"
assert_contains "$sql_stdin" "checkpoint_restore_phase" "checkpoint restore phase query should be present"
assert_contains "$sql_stdin" "runtime_checkpoint_restore_phases.duration_ms" "checkpoint restore phase duration should be reported"
assert_contains "$sql_stdin" "runtime_checkpoint_artifacts.encrypt_duration_ms" "checkpoint artifact encrypt duration should be reported"
assert_contains "$sql_stdin" "runtime_checkpoint_artifacts.store_duration_ms" "checkpoint artifact store duration should be reported"
assert_contains "$sql_stdin" "worker_commands" "worker command evidence query should be present"
