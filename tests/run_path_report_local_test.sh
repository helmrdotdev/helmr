...
assert_contains "$sql_stdin" "WHERE id = '${run_id}'" "run id should be bound into report SQL"
assert_contains "$sql_stdin" "SELECT id FROM runs WHERE id = '${run_id}'" "run id should resolve authority"
...
assert_contains "$sql_stdin" "reclaim_evidence" "runtime cleanup evidence should be present"
...
assert_contains "$tmp/bad-uuid.err" "RUN_ID must be a canonical UUIDv7" "invalid Run ID error"
...
if PATH="$tmp/bin:$PATH" \
	...
	CAPTURED_SQL_STDIN="$sql_stdin" \
	CAPTURED_PSQL_CALLS="$psql_calls" \
	"$repo_root/dev/aws/run-path-report.sh" $'019c10d5-a6f7-7af1-8f5f-bb97bcc0dc31\nignored' \
	>"$tmp/multiline-run.out" 2>"$tmp/multiline-run.err"; then
	fail "multi-line run id should fail"
fi
assert_contains "$tmp/multiline-run.err" "RUN_ID must be a canonical UUIDv7" "multi-line Run ID error"
