...
assert_equal "helmr-worker-${rc2_slug}" "$(identity_value worker_image_name "$tmp/rc2.env")" "rc worker image name"
...
assert_equal "helmr-worker-${stable_slug}" "$(identity_value worker_image_name "$tmp/stable.env")" "stable worker image name"
...
for invalid_tag in v1.2 v01.2.3 v1.2.3-01; do
  if "$repo_root/scripts/release-worker-image-identity.sh" "$invalid_tag" 2>/dev/null; then
    fail "invalid release tag should fail: $invalid_tag"
  fi
done
...
long_name="$("$repo_root/scripts/release-worker-image-identity.sh" \
  ...
  helmr-worker-name-with-a-long-prefix-for-testing \
