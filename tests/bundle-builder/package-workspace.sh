...
build_fixture() {
  ...
  "$shared/helmr" build "$project" --output "$tmp/$label" 2>"$tmp/$label.log" ||
...
export const hello = task({ id: "hello", run: () => "hello" })
TS
"$shared/helmr" build "$mutation_project" \
...
"$shared/helmr" build "$workspace_project" \
...
printf 'ok - bundle builder package-workspace lane\n'
