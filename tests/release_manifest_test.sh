...
controlplane_image="ghcr.io/helmrdotdev/control-plane@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
...
if "${script}" "ghcr.io/helmrdotdev/control-plane:latest" "${worker_image}" "${platform_release}" "${tmp}/tagged-image.json" 2>/dev/null; then
