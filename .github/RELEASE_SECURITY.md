...
Control Plane publishes only complete, closed Platform release objects. It validates
...
For a Deployment, Control Plane submits an exact Node and package-manager selector to
...
conformance checks, and publishes candidates to the Platform store. Control Plane
...
blocked. Control Plane and Workers may create and read immutable objects but have no
...
Control Plane images and Worker AMIs are built directly from the same checked-out
