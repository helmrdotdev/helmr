...
declare const runStreamDefinitionBrand: unique symbol

export interface RunStreamRecord<TData extends JsonValue> {
  readonly id: string
  readonly sequence: number
  readonly data: TData
  readonly contentType: string
...
export interface RunStreamDefinition {
  readonly [runStreamDefinitionBrand]: Readonly<{
    input: JsonValue
    record: JsonValue
  }>
  readonly id: string
...
export interface TypedRunStreamDefinition<
  TInput extends JsonValue,
  TRecord extends JsonValue,
> extends RunStreamDefinition {
  readonly [runStreamDefinitionBrand]: Readonly<{
    input: TInput
    record: TRecord
  }>
  append(
    value: TInput,
    options?: OutputAppendOptions,
  ): Promise<RunStreamRecord<TRecord>>
  pipe(
    source: AsyncIterable<TInput> | Iterable<TInput>,
    options?: OutputSequenceOptions,
  ): Promise<void>
  writer(options?: OutputSequenceOptions): Readonly<{
  ...
  read(
    runId: string,
    options?: OutputReadOptions,
  ): AsyncIterable<RunStreamRecord<TRecord>>
  list(
    runId: string,
    options?: OutputReadOptions,
  ): Promise<readonly RunStreamRecord<TRecord>[]>
}

export type RunStreamInput<TStream extends RunStreamDefinition> =
  TStream[typeof runStreamDefinitionBrand]["input"]
export type RunStreamData<TStream extends RunStreamDefinition> =
  TStream[typeof runStreamDefinitionBrand]["record"]
