...
This layer sits between the SDK-facing task modules and the guest protocol. It loads tasks, compiles task metadata during parse, runs task bodies, and translates task-side effects such as waits, stream records, metadata updates, and logs into the host protocol.
