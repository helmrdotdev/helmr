...
This layer sits between the SDK-facing task modules and the guest protocol. It loads tasks, compiles task metadata during parse, runs task bodies, and translates task-side effects such as waitpoints, session channel output, metadata updates, waits, and logs into the host protocol.
