...
For idle periods, `aws-dev-debug.sh dev-off` scales worker, control, and
...
`aws-dev-debug.sh status` includes ECS, Redis/Valkey, RDS, and worker state. RDS
can restart automatically after seven days when stopped, so destroy long-lived
throwaway stacks instead of parking them.
