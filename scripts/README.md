...
For local dev, you may keep those ClickHouse Cloud inputs in
`.helmr-aws-dev-smoke/clickhouse-cloud.env`; the smoke script reads that file when present and shell
environment variables take precedence:

```sh
mkdir -p .helmr-aws-dev-smoke
chmod 700 .helmr-aws-dev-smoke
cat >.helmr-aws-dev-smoke/clickhouse-cloud.env <<'EOF'
DEV_CREATE_CLICKHOUSE_CLOUD=true
DEV_CLICKHOUSE_ORGANIZATION_ID=...
CLICKHOUSE_CLOUD_API_KEY=...
CLICKHOUSE_CLOUD_API_SECRET=...
EOF
chmod 600 .helmr-aws-dev-smoke/clickhouse-cloud.env
```
