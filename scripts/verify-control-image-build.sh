...
if [ -z "${receipt}" ] || [ -z "${image_uri}" ]; then
  printf 'usage: scripts/verify-control-image-build.sh <build-inputs.json> <image-uri>\n' >&2
  exit 1
fi
[ -f "${receipt}" ] || {
  printf 'Control image build-input receipt is missing: %s\n' "${receipt}" >&2
  exit 1
}

base_image="$(jq -er '.baseImage' "${repo_root}/images/control-image-build.json")"
source_commit="$(git -C "${repo_root}" rev-parse HEAD)"
local_image_id="$("${docker_bin}" image inspect --format '{{.Id}}' "${image_uri}")"
printf '%s\n' "${local_image_id}" | grep -Eq '^sha256:[0-9a-f]{64}$' || {
  printf 'Control image does not have a content-addressed local identity\n' >&2
  exit 1
}
...
' "${receipt}" >/dev/null || {
  printf 'Control image build-input receipt does not match the selected local image and checkout\n' >&2
  exit 1
}
