...
VALUES
    ('00000000-0000-0000-0000-000000001101', 'run_aaaaaaaaaaaaaaaaaaaaaaaaaa', '00000000-0000-0000-0000-000000000201', current_setting('helmr.seed_worker_group_id'), '00000000-0000-0000-0000-000000000301', '00000000-0000-0000-0000-000000000401', '00000000-0000-0000-0000-000000000601', '00000000-0000-0000-0000-000000000811', '00000000-0000-0000-0000-000000000901', 'dev-2026-06-22', 'dev', 'code-review', '00000000-0000-0000-0000-000000001001', 'running', 'executing', NULL, '{"repository":"helmr"}', NULL, '{"summary":"Reviewing workspace runtime branch"}', ARRAY['demo','review'], 'default', 10, 1800000, '11111111111111111111111111111111', '1111111111111111', '00000000-0000-0000-0000-000000001201', 1, NULL, NULL, now() - interval '45 minutes', now() - interval '3 minutes', now() - interval '44 minutes', NULL),
    ...
    ('00000000-0000-0000-0000-000000001103', 'run_aaaaaaaaaaaaaaaaaaaaaaaaac', '00000000-0000-0000-0000-000000000201', current_setting('helmr.seed_worker_group_id'), '00000000-0000-0000-0000-000000000301', '00000000-0000-0000-0000-000000000401', '00000000-0000-0000-0000-000000000601', '00000000-0000-0000-0000-000000000813', '00000000-0000-0000-0000-000000000903', 'dev-2026-06-22', 'dev', 'failure-boundary', '00000000-0000-0000-0000-000000001003', 'failed', 'finished', 'failed', '{"mode":"failure"}', '{"ok":false,"error":"fixture failure"}', '{"summary":"Failed before producing output"}', ARRAY['demo','failed'], 'default', 0, 600000, '33333333333333333333333333333333', '3333333333333333', '00000000-0000-0000-0000-000000001203', 1, 1, 'fixture failure', now() - interval '30 minutes', now() - interval '21 minutes', now() - interval '29 minutes', now() - interval '21 minutes'),
...
INSERT INTO run_attempts (id, org_id, worker_group_id, run_id, attempt_number, status, output, error_message, created_at, updated_at, started_at, finished_at)
VALUES
    ('00000000-0000-0000-0000-000000001201', '00000000-0000-0000-0000-000000000201', current_setting('helmr.seed_worker_group_id'), '00000000-0000-0000-0000-000000001101', 1, 'running', NULL, NULL, now() - interval '45 minutes', now() - interval '3 minutes', now() - interval '44 minutes', NULL),
    ('00000000-0000-0000-0000-000000001202', '00000000-0000-0000-0000-000000000201', current_setting('helmr.seed_worker_group_id'), '00000000-0000-0000-0000-000000001102', 1, 'waiting', NULL, NULL, now() - interval '35 minutes', now() - interval '18 minutes', now() - interval '34 minutes', NULL),
    ('00000000-0000-0000-0000-000000001203', '00000000-0000-0000-0000-000000000201', current_setting('helmr.seed_worker_group_id'), '00000000-0000-0000-0000-000000001103', 1, 'failed', '{"ok":false}', 'fixture failure', now() - interval '30 minutes', now() - interval '21 minutes', now() - interval '29 minutes', now() - interval '21 minutes'),
    ('00000000-0000-0000-0000-000000001204', '00000000-0000-0000-0000-000000000201', current_setting('helmr.seed_worker_group_id'), '00000000-0000-0000-0000-000000001104', 1, 'succeeded', '{"ok":true}', NULL, now() - interval '24 minutes', now() - interval '12 minutes', now() - interval '23 minutes', now() - interval '12 minutes')
ON CONFLICT (id) DO UPDATE
   SET worker_group_id = EXCLUDED.worker_group_id,
       status = EXCLUDED.status,
       output = EXCLUDED.output,
...
VALUES ('00000000-0000-0000-0000-000000001401', 'sch_aaaaaaaaaaaaaaaaaaaaaaaaaa', '00000000-0000-0000-0000-000000000201', '00000000-0000-0000-0000-000000000301', 'imperative', 'release-summary', 'dev-release-summary', 'dev-release-summary', 'dev/release-summary', '*/15 * * * *', 'UTC', true)
ON CONFLICT (id) DO UPDATE
   SET task_id = EXCLUDED.task_id,
       cron = EXCLUDED.cron,
...
INSERT INTO task_schedule_instances (id, schedule_id, org_id, project_id, environment_id, task_id, run_options, enabled, next_fire_at, last_fire_at, last_trigger_run_id)
