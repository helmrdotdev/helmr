import { cache, image, logger, sandbox, source, streams, task, tokens } from "@helmr/sdk"
...
const base = image("helmr-token-checkpoint-smoke")
  ...
  .run(["bun", "install", "--frozen-lockfile"], {
    cache: [{ mountPath: "/root/.bun/install/cache", cache: cache("token-checkpoint-smoke-bun") }],
  })
  .workdir("/workspace")

const sbx = sandbox("helmr-token-checkpoint-smoke")
  .image(base)
...
const statePath = "token-checkpoint-smoke-state.json"
const logPath = "token-checkpoint-smoke.log"
const reportPath = "token-checkpoint-smoke-report.json"
...
const tokenEvents = streams.output("token-checkpoint-smoke.tokens", { schema: z.unknown() })

export const tokenCheckpointSmoke = task({
  ...
  run: async (payload: Payload, ctx) => {
    ...
    logger.info({ phase: "token-checkpoint-smoke", step: "before-decision", marker, pid: process.pid })
    ...
    await tokenEvents.append({
      marker,
      step: "decision",
      tokenId: decisionToken.id,
    }, { contentType: "application/vnd.helmr.token-checkpoint-smoke.token+json" })
    ...
    logger.info({ phase: "token-checkpoint-smoke", step: "before-input", marker, pid: process.pid })
    ...
    await tokenEvents.append({
      marker,
      step: "reply",
      tokenId: replyToken.id,
    }, { contentType: "application/vnd.helmr.token-checkpoint-smoke.token+json" })
    const reply = await replyToken.wait({
    ...
    logger.info({ phase: "token-checkpoint-smoke", step: "completed", marker, pid: process.pid })
    return report
  },
})
