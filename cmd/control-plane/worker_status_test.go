...
func TestWorkerStatusCommandsRejectMalformedGroupBeforePersistence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "group status", run: func() error {
			return runWorkerGroupStatusCommand(context.Background(), io.Discard, []string{"status", "--group-id", "not-a-group"})
		}},
