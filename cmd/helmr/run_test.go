...
func TestRunStartCreatesTaskRun(t *testing.T) {
...
func TestRunStartOmitsPayloadWhenNotSpecified(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost || r.URL.Path != "/api/tasks/deploy/start" {
			t.Fatalf("%s %s", r.Method, r.URL.Path)
		}
	...
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "deploy", "--workspace", "ws_1"})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
}
...
func TestRunCommandReadsPayloadFile(t *testing.T) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "deploy", "--workspace", "ws_1", "--payload-file", payloadPath})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
...
func TestRunStartRequiresExistingWorkspace(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "deploy"})
	if err := cmd.Execute(); err == nil || !strings.Contains(err.Error(), "--workspace is required") {
		t.Fatalf("err = %v", err)
	}
}

func TestRunStartWaitWaitsForRun(t *testing.T) {
	getRunCalls := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
	...
	cmd.SetOut(&out)
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "deploy", "--workspace", "ws_1", "--wait", "--timeout", "1500ms"})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
	if getRunCalls != 1 {
...
func TestRunStartWaitPollsRunSnapshot(t *testing.T) {
	getRunCalls := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodPost && r.URL.Path == "/api/tasks/deploy/start":
...
func TestRunStartRejectsJSONFollow(t *testing.T) {
	called := false
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		called = true
	}))
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "deploy", "--workspace", "ws_1", "--json", "--follow"})
	err := cmd.Execute()
...
func TestRunStartFollowTimeoutReturnsError(t *testing.T) {
	...
	cmd.SetOut(&out)
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "deploy", "--workspace", "ws_1", "--follow", "--timeout", "1s"})
	err := cmd.Execute()
...
func TestRunCommandRejectsPayloadFileCombinations(t *testing.T) {
	...
	for _, args := range [][]string{
		{"run", "start", "deploy", "--workspace", "ws_1", "--payload-file", payloadPath, "--payload-json", `{"env":"prod"}`},
		{"run", "start", "deploy", "--workspace", "ws_1", "--payload-file", payloadPath, "--payload", "env=prod"},
	} {
		cmd := newRootCommand()
		cmd.SetOut(&bytes.Buffer{})
		cmd.SetErr(&bytes.Buffer{})
...
func TestRunCommandDoesNotExposeInputFlagAliases(t *testing.T) {
	for _, args := range [][]string{
		{"run", "start", "deploy", "--workspace", "ws_1", "--input-json", `{"env":"prod"}`},
		{"run", "start", "deploy", "--workspace", "ws_1", "--input-file", "payload.json"},
		{"run", "start", "deploy", "--workspace", "ws_1", "--input", "env=prod"},
	} {
		cmd := newRootCommand()
		cmd.SetOut(&bytes.Buffer{})
		cmd.SetErr(&bytes.Buffer{})
...
func TestRunCommandRejectsProjectFlagThatLooksLikePayload(t *testing.T) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "deploy", "--workspace", "ws_1", "-p", "env=prod"})
	err := cmd.Execute()
...
func TestRunCommandRejectsInvalidTaskIDBeforeRequest(t *testing.T) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"run", "start", "bad task", "--workspace", "ws_1"})
	err := cmd.Execute()
