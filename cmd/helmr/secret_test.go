...
const testSecretID = "019c10d5-a6f7-7af1-8f5f-bb97bcc0dc38"
...
func TestSecretGetCommandReturnsMetadataOnly(t *testing.T) {
	...
	cmd.SetOut(&out)
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"secret", "get", testSecretID, "--project", "project-1", "--env", "env-1"})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
...
func TestSecretRevokeCommand(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(api.SecretResponse{Name: "github-token", Status: "revoked"})
	}))
	defer server.Close()
	t.Setenv(helmrAPIURLEnv, server.URL)
	if err := state.SaveLogin(server.URL, "session-test"); err != nil {
...
func TestSecretRevokeCommandRequiresYes(t *testing.T) {
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"secret", "revoke", testSecretID})
	err := cmd.Execute()
