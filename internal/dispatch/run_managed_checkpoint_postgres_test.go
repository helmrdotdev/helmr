		...
		t.Run(fmt.Sprintf("actor_%t", actor), func(t *testing.T) {
			for _, mode := range []string{"valid timer", "wrong parent", "open source runtime", "failed unreclaimed", "failed reclaimed", "lost reclaimed", "not materialized"} {
				t.Run(mode, func(t *testing.T) {
					...
					switch mode {
					case "wrong parent":
						dbtest.MustExec(t, f.ctx, f.pool, `WITH other_head AS (INSERT INTO computer_versions(id,environment_id,computer_id,status,root_pack_digest,logical_bytes,ownership_generation,writer_generation,published_at,parent_version_id,source_workspace_lease_id) SELECT gen_random_uuid(),v.environment_id,v.computer_id,'committed',v.root_pack_digest,v.logical_bytes,v.ownership_generation,v.writer_generation,now(),v.parent_version_id,v.source_workspace_lease_id FROM computer_versions v JOIN run_checkpoints c ON c.private_workspace_version_id=v.id WHERE c.id=$1 RETURNING id) UPDATE computer_versions SET parent_version_id=(SELECT id FROM other_head) WHERE id=(SELECT private_workspace_version_id FROM run_checkpoints WHERE id=$1)`, checkpointID)
