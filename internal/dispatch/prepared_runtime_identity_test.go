...
func TestPreparedRuntimeKeyFromSourceMatchesGolden(t *testing.T) {
	key := preparedRuntimeKeyFromSource(goldenPreparedRuntimeSource(), compute.NetworkPolicy{Internet: false, Deny: []string{"10.0.0.0/8"}})
...
func TestPreparedRuntimeKeyFromSourceMatchesZeroNetworkGolden(t *testing.T) {
	key := preparedRuntimeKeyFromSource(goldenPreparedRuntimeSource(), compute.NetworkPolicy{})
...
func TestPreparedRuntimeKeyFromSourceSwallowsSubstrateKeyError(t *testing.T) {
	source := goldenPreparedRuntimeSource()
	source.SandboxImageArtifact.Digest = ""
	key := preparedRuntimeKeyFromSource(source, compute.NetworkPolicy{Internet: false, Deny: []string{"10.0.0.0/8"}})
...
func goldenPreparedRuntimeSource() api.WorkerPreparedRuntimeSource {
