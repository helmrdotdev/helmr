...
func TestVMEngineExecutesExactOneShotImageBuild(t *testing.T) {
	...
	paths := []imagebuild.SourcePath{
		{Path: "package.json", Kind: imagebuild.SourcePathFile},
		{Path: "src", Kind: imagebuild.SourcePathDirectory},
	}
	...
	resolution := imagebuild.RegistryBinding{
	...
	admission := workerTestAdmission{admit: func(request AdmissionRequest) (Assignment, error) {
		...
		assignment.RegistryBindings = []imagebuild.RegistryBinding{resolution}
		assignment.ResolutionSetDigest = imagebuild.ResolutionSetDigest([]imagebuild.RegistryBinding{resolution})
		assignment.CacheBinding = &imagebuild.CacheBinding{
	...
	credentials := workerTestCredentials{fetch: func(request RegistryCredentialRequest) ([]imagebuild.RegistryCredentialValue, error) {
		...
		return []imagebuild.RegistryCredentialValue{{
			Authority: "docker.io", Username: "registry-user", Password: userPassword,
		}}, nil
	}}
	cache := workerTestCache{fetch: func(assignment Assignment) (imagebuild.RegistryCredentialValue, error) {
	...
	request := validBuildRequest(t, plan, source)
	request.RequestedCacheMode = imagebuild.CachePrefer
...
func TestVMEngineRejectsAssignmentThatChangesAdmittedFactsBeforeVM(t *testing.T) {
	...
	if _, err := engine.BuildWorkspaceImage(t.Context(), validBuildRequest(t, plan, source), &workerTestRevocations{}); err == nil {
...
func TestVMEngineReplaysTerminalSuccessWithoutVMOrCredentials(t *testing.T) {
	...
	engine := VMEngine{
		...
		Credentials: workerTestCredentials{fetch: func(RegistryCredentialRequest) ([]imagebuild.RegistryCredentialValue, error) {
	...
	revocations := &workerTestRevocations{}
	artifact, err := engine.BuildWorkspaceImage(t.Context(), validBuildRequest(t, plan, source), revocations)
	if err != nil {
		t.Fatal(err)
	}
...
func TestVMEngineRevocationCancelsAndDestroysOnlyLiveAttempt(t *testing.T) {
	...
	go func() {
		_, err := engine.BuildWorkspaceImage(context.Background(), validBuildRequest(t, plan, source), revocations)
		done <- err
	}()
...
func TestVMEngineCompletesGuestFailureBeforeReturningIt(t *testing.T) {
	...
	if len(completion.requests) != 1 || completion.requests[0].Artifact != nil ||
		...
		completion.requests[0].Evidence.GuestResult.Outcome != imagebuild.GuestFailed {
...
func TestVMEngineNetworkQuotaOverridesGuestFailure(t *testing.T) {
	...
	if session.statusCalls != 1 || len(completion.requests) != 1 ||
		completion.requests[0].Evidence.GuestResult.FailureReason != imagebuild.GuestFailureNetworkQuota {
...
func TestVMEngineRunsColdWhenCacheCredentialIsUnavailableBeforeDelivery(t *testing.T) {
	...
	stream.inspect = func(raw []byte) {
		...
		if request.RequestedCacheMode != imagebuild.CachePrefer || request.CacheBinding != nil {
	...
	engine := VMEngine{
		...
		Cache: workerTestCache{fetch: func(Assignment) (imagebuild.RegistryCredentialValue, error) {
			return imagebuild.RegistryCredentialValue{}, &imagecache.UnavailableError{
...
func TestVMEngineRejectsCacheCredentialContractFailureBeforeDelivery(t *testing.T) {
	...
	engine := VMEngine{
		...
		Credentials: workerTestCredentials{},
		Cache: workerTestCache{fetch: func(Assignment) (imagebuild.RegistryCredentialValue, error) {
			return imagebuild.RegistryCredentialValue{}, &imagecache.ContractError{Message: "target mismatch"}
		}},
		Completion: &workerTestCompletion{}, WorkDir: t.TempDir(),
	}
	request := validBuildRequest(t, plan, source)
...
func (source *workerTestSource) Paths() ([]imagebuild.SourcePath, error) {
...
type workerTestCredentials struct {
	fetch func(RegistryCredentialRequest) ([]imagebuild.RegistryCredentialValue, error)
}

type blockingWorkerTestCredentials struct{}

func (blockingWorkerTestCredentials) FetchRegistryCredentials(ctx context.Context, _ RegistryCredentialRequest) ([]imagebuild.RegistryCredentialValue, error) {
	<-ctx.Done()
	return nil, ctx.Err()
}

func (fetcher workerTestCredentials) FetchRegistryCredentials(_ context.Context, request RegistryCredentialRequest) ([]imagebuild.RegistryCredentialValue, error) {
	if fetcher.fetch == nil {
		return []imagebuild.RegistryCredentialValue{}, nil
	}
	return fetcher.fetch(request)
}

type workerTestCache struct {
	fetch func(Assignment) (imagebuild.RegistryCredentialValue, error)
}
...
func (provider workerTestCache) FetchImageCacheCredential(_ context.Context, assignment Assignment) (imagebuild.RegistryCredentialValue, error) {
...
func validBuildRequest(t *testing.T, plan imagebuild.Build, source SourceArchive) BuildRequest {
	...
	return BuildRequest{
		...
		RequestedCacheMode: imagebuild.CacheBypass, Source: source,
	}
}

func validWorkerBuild() imagebuild.Build {
		...
		Images: []imagebuild.Spec{
			{
				...
				Steps: []imagebuild.Step{
					...
					{Run: &imagebuild.Run{Argv: []string{"sh", "-c", "mkdir -p /app /out"}}},
					{CopySourceFile: &imagebuild.CopySourceFile{Dst: "/app/package.json", Path: "package.json"}},
					{CopySourceDir: &imagebuild.CopySourceDir{Dst: "/app/src", Path: "src"}},
					{Workdir: &imagebuild.Workdir{Path: "/app"}},
					{User: &imagebuild.User{Name: "1000:1000"}},
					{Env: &imagebuild.Env{Key: "NODE_ENV", Value: "production"}},
					{Env: &imagebuild.Env{Key: "EMPTY", Value: ""}},
					{CopyFromImage: &imagebuild.CopyFromImage{Dst: "/usr/local/bin/tool", ImageKey: "base", SrcPath: "/out/tool"}},
				},
			},
			{
				Key:      "base",
...
func validWorkerAssignment(request AdmissionRequest, operationID string) Assignment {
	...
	return Assignment{
		...
		RegistryBindings: []imagebuild.RegistryBinding{}, ResolutionSetDigest: imagebuild.ResolutionSetDigest([]imagebuild.RegistryBinding{}),
...
func parseWorkerTestRequest(t *testing.T, raw []byte) (imagebuild.GuestRequest, imagebuild.CredentialEnvelope, []byte) {
	t.Helper()
	...
	requestRaw, err := frameio.ReadMessageFrameBounded(reader, imagebuild.RequestDocumentMaxBytes)
	if err != nil {
		t.Fatal(err)
	}
	request, err := imagebuild.ParseGuestRequest(requestRaw)
	if err != nil {
		t.Fatal(err)
	}
	...
	envelopeRaw, err := frameio.ReadMessageFrameBounded(reader, imagebuild.CredentialEnvelopeMaxBytes)
	if err != nil {
		t.Fatal(err)
	}
