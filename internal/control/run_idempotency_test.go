...
func TestCreateRunReturnsExistingRunForActiveIdempotencyKey(t *testing.T) {
	...
	if len(store.sessionStartKeys) != 1 || len(store.sessionStartKeys[0].IdempotencyKey) != sha256.Size*2 {
		t.Fatalf("stored idempotency keys = %+v", store.sessionStartKeys)
	}
...
func TestCreateRunRequiresCoordinationBeforeBindingExistingExternalIDToIdempotencyKey(t *testing.T) {
	...
	requireErrorCode(t, rec.Body.Bytes(), "coordination_unavailable")
	if len(store.sessionStartKeys) != 0 {
		t.Fatalf("start idempotency binding was written without Redis coordination: %+v", store.sessionStartKeys)
	}
}
...
func TestCreateRunBindsIdempotencyKeyWhenExternalIDReusesSession(t *testing.T) {
	...
	if len(store.sessionStartKeys) != 1 || store.sessionStartKeys[0].SessionID != store.session.ID || store.sessionStartKeys[0].RunID != firstRunID {
		t.Fatalf("stored idempotency keys = %+v session=%s run=%s", store.sessionStartKeys, pgvalue.MustUUIDValue(store.session.ID), pgvalue.MustUUIDValue(firstRunID))
	}
...
func TestCreateRunBindsIdempotencyKeyAfterExternalIDUniqueRace(t *testing.T) {
	...
	if len(store.sessionStartKeys) != 1 || store.sessionStartKeys[0].SessionID != sessionID || store.sessionStartKeys[0].RunID != runID {
		t.Fatalf("idempotency binding = %+v, want session %s run %s", store.sessionStartKeys, pgvalue.MustUUIDValue(sessionID), pgvalue.MustUUIDValue(runID))
	}
}
...
func TestCreateRunReclaimsExpiredStartIdempotency(t *testing.T) {
	...
	store.sessionStartKeys[0].ExpiresAt = pgtype.Timestamptz{Time: time.Now().Add(-time.Second), Valid: true}
...
func TestCreateRunHashesLiteralHexIdempotencyKeys(t *testing.T) {
	...
	if got, want := store.sessionStartKeys[0].IdempotencyKey, hex.EncodeToString(digest[:]); got != want {
