...
func seedControlStreamTokenFixture(t *testing.T, ctx context.Context, pool *pgxpool.Pool) streamTokenRouteFixture {
	...
	if _, err := pool.Exec(ctx, `INSERT INTO deployments (id, public_id, org_id, build_cell_id, project_id, environment_id, worker_group_id, version, content_hash, deployment_source_artifact_id, status) VALUES ($1, $9, $2, $3, $4, $5, $6, 'v1', $7, $8, 'deployed')`, ids.deploymentID, ids.orgID, dbtest.DefaultCellID, ids.projectID, ids.environmentID, ids.workerGroupID, digest, artifactID, streamTestPublicID(t, publicid.Deployment)); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO deployment_sandboxes (id, public_id, org_id, project_id, environment_id, deployment_id, sandbox_id, image_artifact_id, image_artifact_cell_id, image_artifact_format, rootfs_digest, image_digest, image_format, workspace_mount_path, runtime_abi, guestd_abi, adapter_abi, filesystem_format, contract_version, fingerprint) VALUES ($1, $9, $2, $3, $4, $5, 'default', $6, $7, 'oci-tar', $8, $8, 'oci-tar', '/workspace', 'test', 'guestd-test', 'adapter-test', 'tar', 1, 'sandbox-fingerprint')`, sandboxID, ids.orgID, ids.projectID, ids.environmentID, ids.deploymentID, taskBundleID, dbtest.DefaultCellID, rootfsDigest, streamTestPublicID(t, publicid.Sandbox)); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `INSERT INTO deployment_tasks (id, public_id, org_id, project_id, environment_id, deployment_id, deployment_sandbox_id, task_id, bundle_artifact_id, bundle_artifact_cell_id, queue_name, max_active_duration_ms) VALUES ($1, $10, $2, $3, $4, $5, $6, $7, $8, $9, 'default', 300000)`, ids.deploymentTaskID, ids.orgID, ids.projectID, ids.environmentID, ids.deploymentID, sandboxID, taskID, artifactID, dbtest.DefaultCellID, streamTestPublicID(t, publicid.DeploymentTask)); err != nil {
		t.Fatal(err)
	}
