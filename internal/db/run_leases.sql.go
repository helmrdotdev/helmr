...
type RecoverExpiredRecreatedRunResumesRow struct {
	ID    pgtype.UUID `json:"id"`
	OrgID pgtype.UUID `json:"org_id"`
	RunID pgtype.UUID `json:"run_id"`
}

func (q *Queries) RecoverExpiredRecreatedRunResumes(ctx context.Context, limitCount int32) ([]RecoverExpiredRecreatedRunResumesRow, error) {
	rows, err := q.db.Query(ctx, recoverExpiredRecreatedRunResumes, limitCount)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []RecoverExpiredRecreatedRunResumesRow
	for rows.Next() {
		var i RecoverExpiredRecreatedRunResumesRow
