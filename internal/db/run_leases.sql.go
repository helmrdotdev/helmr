...
abandoned_runtime_checkpoint_restore AS (
    UPDATE runtime_checkpoint_restores
     ...
     WHERE runtime_checkpoint_restores.org_id = $1
       ...
       AND runtime_checkpoint_restores.run_lease_id = $2
...
retry_plan AS (
    SELECT retry_candidate.run_id, retry_candidate.org_id, retry_candidate.worker_group_id, retry_candidate.project_id, retry_candidate.environment_id, retry_candidate.previous_attempt_id, retry_candidate.previous_attempt_number, retry_candidate.next_attempt_id, retry_candidate.next_attempt_number, retry_candidate.reason, retry_candidate.delay_ms, retry_candidate.retry_after, retry_candidate.locked_retry_policy
...
locked_dispatch AS MATERIALIZED (
    SELECT run_queue_items.run_id,
     ...
     WHERE run_queue_items.org_id = $1
       ...
       AND run_queue_items.dispatch_message_id = $4::text
       AND run_queue_items.status = 'reserved'
       AND run_queue_items.reservation_expires_at > now()
    ...
    SELECT runtime_resume_capacity.id AS run_id,
      ...
      CROSS JOIN LATERAL (
          SELECT pg_advisory_xact_lock(
                     hashtext($1::text || ':' || runtime_resume_capacity.worker_group_id || ':' || runtime_resume_capacity.environment_id::text),
...
requeued_attempts AS (
    UPDATE run_attempts
      ...
      FROM updated_runs
     WHERE run_attempts.org_id = $1
       AND run_attempts.worker_group_id = $2
       AND run_attempts.run_id = updated_runs.run_id
...
abandoned_runtime_checkpoint_restore AS (
    UPDATE runtime_checkpoint_restores
      ...
      FROM updated_runs
     WHERE runtime_checkpoint_restores.org_id = $1
       AND runtime_checkpoint_restores.worker_group_id = $2
       AND runtime_checkpoint_restores.run_id = updated_runs.run_id
       AND runtime_checkpoint_restores.run_lease_id = updated_runs.run_lease_id
       AND runtime_checkpoint_restores.runtime_checkpoint_id = updated_runs.restore_runtime_checkpoint_id
       AND runtime_checkpoint_restores.status = 'restoring'
...
requeued_queue_entries AS (
    UPDATE run_queue_items
      ...
      JOIN run_leases ON run_leases.org_id = $1
                         ...
                         AND run_leases.id = updated_runs.run_lease_id
     WHERE run_queue_items.org_id = $1
       AND run_queue_items.worker_group_id = $2
       AND run_queue_items.run_id = updated_runs.run_id
       AND run_queue_items.reserved_by_worker_instance_id = run_leases.worker_instance_id
       AND run_queue_items.dispatch_message_id = run_leases.dispatch_message_id
       AND run_queue_items.status = 'reserved'
...
lost_event_seq AS (
    INSERT INTO event_cursors (org_id, worker_group_id, subject_kind, subject_id, seq)
    SELECT $1, updated_runs.worker_group_id, 'run', updated_runs.run_id, 1
      FROM updated_runs
    ON CONFLICT (org_id, worker_group_id, subject_kind, subject_id)
