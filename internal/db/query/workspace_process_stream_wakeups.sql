...
INSERT INTO workspace_process_stream_wakeups (
    org_id,
    worker_group_id,
    project_id,
    environment_id,
...
WITH claimable AS (
    SELECT id
      FROM workspace_process_stream_wakeups
...
updated AS (
    UPDATE workspace_process_stream_wakeups
       SET locked_until = now() + sqlc.arg(lease_duration)::interval,
           attempts = workspace_process_stream_wakeups.attempts + 1
      FROM claimable
     WHERE workspace_process_stream_wakeups.id = claimable.id
    RETURNING workspace_process_stream_wakeups.*
)
SELECT *
  FROM updated
 ORDER BY id ASC;
...
DELETE FROM workspace_process_stream_wakeups
...
UPDATE workspace_process_stream_wakeups
 ...
 WHERE workspace_process_stream_wakeups.id = sqlc.arg(id)
