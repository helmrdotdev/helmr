...
UPDATE runtime_instances r
   SET computer_save_sequence=sqlc.arg(sequence),
       computer_save_id=sqlc.arg(save_id),
       computer_save_lease_id=sqlc.arg(lease_id),
       computer_save_predecessor_id=sqlc.arg(predecessor_id),
 ...
 WHERE r.id=sqlc.arg(runtime_instance_id)
   ...
   AND (
       (r.computer_save_id IS NULL AND r.computer_save_sequence=sqlc.arg(sequence)::bigint-1
       ...
       OR (r.computer_save_sequence=sqlc.arg(sequence) AND r.computer_save_id=sqlc.arg(save_id)
           AND r.computer_save_lease_id=sqlc.arg(lease_id)
           AND r.computer_save_predecessor_id=sqlc.arg(predecessor_id))
   )
...
UPDATE runtime_instances r
 ...
 WHERE r.id=sqlc.arg(runtime_instance_id)
   AND r.worker_instance_id=sqlc.arg(worker_instance_id) AND r.worker_epoch=sqlc.arg(worker_epoch)
   AND r.computer_save_sequence=sqlc.arg(sequence) AND r.computer_save_id=sqlc.arg(save_id)
   AND r.computer_save_lease_id=sqlc.arg(lease_id)
...
WITH created AS (
 ...
 sqlc.arg(content_digest),sqlc.arg(logical_bytes),'committed',l.id,l.ownership_generation,l.writer_generation,
 ...
 WHERE r.id=sqlc.arg(runtime_instance_id) AND r.computer_save_id=sqlc.arg(save_id)
 ...
 SELECT environment_id,workspace_id,id,sqlc.arg(locator) FROM created RETURNING version_id
), advanced AS (
...
SELECT v.* FROM created v JOIN advanced c ON c.id=v.workspace_id;
...
SELECT computer_save_sequence > sqlc.arg(sequence)::bigint
   OR (computer_save_sequence = sqlc.arg(sequence)::bigint
       AND computer_save_id IS DISTINCT FROM sqlc.arg(save_id)::uuid) AS adopted
...
SELECT (r.computer_save_sequence >= sqlc.arg(sequence)::bigint
        AND (r.computer_save_sequence > sqlc.arg(sequence)::bigint OR r.computer_save_id IS NULL)
