...
instance_inputs AS (
    SELECT sqlc.arg(instance_id) AS id,
           ...
           sqlc.arg(instance_active) AS enabled,
           CASE WHEN sqlc.arg(instance_active) THEN sqlc.arg(next_fire_at)::timestamptz ELSE NULL END AS next_fire_at
      FROM schedule
    UNION ALL
    SELECT uuidv7() AS id,
           ...
           CASE WHEN task_schedule_instances.enabled THEN sqlc.arg(next_fire_at)::timestamptz ELSE NULL END AS next_fire_at
...
updated_instances AS (
    UPDATE task_schedule_instances
       SET enabled = sqlc.arg(instance_active),
           generation = task_schedule_instances.generation + 1,
...
updated_current_instance AS (
    UPDATE task_schedule_instances
       SET run_options = sqlc.arg(run_options)::jsonb,
           task_id = updated_schedule.task_id,
           enabled = sqlc.arg(instance_active),
           generation = task_schedule_instances.generation + 1,
           next_fire_at = CASE WHEN sqlc.arg(instance_active) THEN sqlc.arg(next_fire_at)::timestamptz ELSE NULL END,
           retry_after = NULL,
           trigger_attempt_count = 0,
           trigger_error_message = '',
           trigger_error_kind = '',
...
retimed_instances AS (
        UPDATE task_schedule_instances
           SET task_id = updated_schedule.task_id,
               generation = task_schedule_instances.generation + 1,
           next_fire_at = CASE WHEN retime_candidates.enabled THEN sqlc.arg(next_fire_at)::timestamptz ELSE NULL END,
           retry_after = NULL,
           trigger_attempt_count = 0,
           trigger_error_message = '',
           trigger_error_kind = '',
...
UPDATE task_schedule_instances
 ...
 WHERE task_schedule_instances.id = sqlc.arg(instance_id)
   AND task_schedule_instances.generation = sqlc.arg(generation)
   AND task_schedule_instances.next_fire_at = sqlc.arg(last_fire_at)
   AND task_schedule_instances.enabled
   AND EXISTS (
       SELECT 1
         FROM task_schedules
         JOIN environments ON environments.org_id = task_schedules.org_id
                              AND environments.project_id = task_schedules.project_id
                              AND environments.id = task_schedule_instances.environment_id
