...
func TestSchemaWorkspaceVersionArtifactAndFinalizationAuthority(t *testing.T) {
	...
	dbtest.MustExec(t, ctx, tx, `WITH lifetime AS (INSERT INTO cas_blobs (digest, size_bytes) VALUES ($2, 1) ON CONFLICT DO NOTHING) INSERT INTO cas_objects (org_id,digest,size_bytes,media_type) VALUES ($1,$2,1,'application/octet-stream')`, fixture.orgID, digest)
