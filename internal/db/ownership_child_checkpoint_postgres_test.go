...
func TestOwnershipDelayedChildBindingUsesLastEligibleWait(t *testing.T) {
	...
	var version int64
	if err := f.pool.QueryRow(ctx, "SELECT revision FROM runs WHERE id=$1", work.runID).Scan(&version); err != nil {
		t.Fatal(err)
	}
	w, err := f.queries.RegisterSameWorkspaceChildCall(ctx, RegisterSameWorkspaceChildCallParams{ID: pgvalue.UUID(uuid.NewV7()), ChildTargetDeclaredID: pgvalue.Text("test-task"), ChildClaimID: pgvalue.UUID(claimID), ChildRequest: []byte("{}"), RegistrationRequestFingerprint: pgvalue.Text(dbtest.Digest("delayed-child")), AttemptNumber: 1, CurrentRunLeaseID: pgvalue.UUID(work.leaseID), ResumeAttachID: pgvalue.UUID(uuid.NewV7()), RunID: pgvalue.UUID(work.runID), EnvironmentID: pgvalue.UUID(f.environmentID), WorkspaceID: v.WorkspaceID, ExpectedRunningRevision: version})
	if err != nil {
		t.Fatal(err)
	}
 ...
 SELECT $1,org_id,project_id,environment_id,deployment_id,deployment_definition_id,'task',entrypoint_declared_id,'child',id,true,$2,workspace_id,$3,'{}',queue_name,now(),now(),max_active_duration_ms,retry_policy,root_span_id FROM runs WHERE id=$4 RETURNING id,workspace_id,base_workspace_version_id) INSERT INTO run_attempts(run_id,number,entrypoint_kind,workspace_id,base_workspace_version_id) SELECT id,1,'task',workspace_id,base_workspace_version_id FROM child`, childID, claimID, v.ID, w.RunID)
	p := CommitSameWorkspaceChildCheckpointReadyParams{CheckpointRequestVersion: 1, BaseWorkspaceVersionID: v.ID, BaseWorkspaceContentDigest: v.ContentDigest, OwnershipGeneration: pgtype.Int8{Int64: v.OwnershipGeneration, Valid: true}, ParentWriterGeneration: pgtype.Int8{Int64: v.WriterGeneration, Valid: true}, CheckpointedAt: pgvalue.Timestamptz(time.Now()), RunWaitID: w.ID, EnvironmentID: pgvalue.UUID(f.environmentID), ParentRunID: w.RunID, WorkspaceID: v.WorkspaceID, ParentAttemptNumber: 1, ChildClaimID: pgvalue.UUID(claimID), ParentRunLeaseID: w.CurrentRunLeaseID, SuspendCheckpointID: pgvalue.UUID(checkpointID), ExpectedRunRevision: w.ExpectedRunRevision, ChildRunID: pgvalue.UUID(childID)}
	...
	for _, sql := range []string{"UPDATE runs SET parent_owns_lifecycle=false WHERE id=$1", "UPDATE run_waits SET expected_run_revision=expected_run_revision+1 WHERE id=$1"} {
		id := p.ChildRunID
