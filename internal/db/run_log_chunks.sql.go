...
seed_run_log_cursor AS (
    ...
    SELECT org_id,
           ...
           0,
           'rlc1.' || org_id::text || '.' || id::text || '.' || ($7::run_log_stream)::text || '.0',
           '__head__'
...
existing_cursor AS (
    SELECT run_log_cursors.org_id,
     ...
     WHERE run_log_cursors.idempotency_key = 'log:' || $5::text || ':' || ($7::run_log_stream)::text || ':' || ($8::bigint)::text
),
inserted_cursor AS (
    ...
    SELECT locked_run_log_cursor.org_id,
           ...
           'log:' || current_run_lease.run_lease_id::text || ':' || locked_run_log_cursor.stream_name || ':' || ($8::bigint)::text
