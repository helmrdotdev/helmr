...
// versions:
//   sqlc v1.31.1
// source: workspace_process_command.sql
...
const clearExpiredWorkspaceExecIdempotency = `-- name: ClearExpiredWorkspaceExecIdempotency :exec
UPDATE workspace_processes
   SET idempotency_key = '',
...
type ClearExpiredWorkspaceExecIdempotencyParams struct {
	OrgID          pgtype.UUID `json:"org_id"`
	ProjectID      pgtype.UUID `json:"project_id"`
	EnvironmentID  pgtype.UUID `json:"environment_id"`
	WorkspaceID    pgtype.UUID `json:"workspace_id"`
...
func (q *Queries) ClearExpiredWorkspaceExecIdempotency(ctx context.Context, arg ClearExpiredWorkspaceExecIdempotencyParams) error {
	_, err := q.db.Exec(ctx, clearExpiredWorkspaceExecIdempotency,
		arg.OrgID,
		arg.ProjectID,
		arg.EnvironmentID,
		arg.WorkspaceID,
...
const getWorkspaceExecByIdempotency = `-- name: GetWorkspaceExecByIdempotency :one
SELECT id, org_id, project_id, environment_id, workspace_id, region_id, worker_group_id, worker_instance_id, worker_epoch, runtime_instance_id, workspace_mount_id, instance_lease_id, write_lease_id, kind, command, cwd, env_shape, filesystem_mode, state, detached, idempotency_key, idempotency_expires_at, request_fingerprint, runtime_process_id, exit_code, signal, pty_cols, pty_rows, pending_pty_cols, pending_pty_rows, stdout_cursor, stderr_cursor, stdin_cursor, stdin_delivered_cursor, stdin_closed_at, input_cursor, input_delivered_cursor, output_cursor, created_by_subject_type, created_by_subject_id, created_at, started_at, exited_at, terminal_at, terminal_reason_code, terminal_error, updated_at
  FROM workspace_processes
 WHERE org_id = $1
   AND project_id = $2
...
type GetWorkspaceExecByIdempotencyParams struct {
	OrgID          pgtype.UUID `json:"org_id"`
	ProjectID      pgtype.UUID `json:"project_id"`
	EnvironmentID  pgtype.UUID `json:"environment_id"`
	WorkspaceID    pgtype.UUID `json:"workspace_id"`
...
func (q *Queries) GetWorkspaceExecByIdempotency(ctx context.Context, arg GetWorkspaceExecByIdempotencyParams) (WorkspaceProcess, error) {
	row := q.db.QueryRow(ctx, getWorkspaceExecByIdempotency,
		arg.OrgID,
		arg.ProjectID,
		arg.EnvironmentID,
		arg.WorkspaceID,
...
WITH inserted AS (
    INSERT INTO workspace_process_stream_chunks (
        ...
        $5,
        $6::text,
        CASE WHEN $6::text = 'stdin' THEN 'input' ELSE 'output' END,
        $7,
        $8,
        $9,
        coalesce($10, now())
