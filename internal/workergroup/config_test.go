...
func TestConfigPrepareResolvesEnrollmentSecretWithoutInfrastructureAuthority(t *testing.T) {
	groups, err := DecodeConfig(`[{"id":"run-workers","name":"Run workers","enrollment_secret_env":"WORKER_GROUP_ENROLLMENT_SECRET_RUN","allows_run":true,"allows_build":false,"observation_ttl_seconds":120,"instance_capacity":{"milli_cpu":1000,"memory_bytes":1024,"guest_ephemeral_disk_bytes":2048,"vm_slots":1}}]`)
	if err != nil {
		t.Fatal(err)
	}
	desired, secret, err := groups[0].Prepare(func(name string) (string, bool) {
		if name != "WORKER_GROUP_ENROLLMENT_SECRET_RUN" {
...
func TestConfigPrepareRequiresExplicitSecretEnvironment(t *testing.T) {
	group := Config{
		ID: "run-workers", Name: "Run workers", EnrollmentSecretEnv: "WORKER_GROUP_ENROLLMENT_SECRET_RUN",
	...
	if _, _, err := group.Prepare(func(string) (string, bool) { return "", true }); err == nil {
		t.Fatal("empty enrollment secret was accepted")
	}
}
