...
func TestWritableDiskJailPreservesCaptureIdentity(t *testing.T) {
	...
	if err := linkWritableDiskIntoJail(source, dir, "disk.ext4", os.Getuid(), os.Getgid()); err != nil {
		t.Fatal(err)
	}
	...
	if err := linkWritableDiskIntoJail(source, dir, "occupied", os.Getuid(), os.Getgid()); !errors.Is(err, os.ErrExist) {
	...
	if err := linkWritableDiskIntoJail(link, dir, "bad", os.Getuid(), os.Getgid()); err == nil {
...
func TestWritableDiskJailRejectsCrossFilesystemCopy(t *testing.T) {
	...
	if err := linkWritableDiskIntoJail(source, root, "disk", os.Getuid(), os.Getgid()); !errors.Is(err, syscall.EXDEV) {
