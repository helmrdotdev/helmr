...
// LayoutContext validates a producer-private OCI layout exported by an earlier
// solve (the installed tree, or the materialized build environment) and returns
// the exact digest-bound BuildKit named-context reference every later solve
// uses, so no later graph can observe different bytes.
func LayoutContext(directory string) (string, error) {
	if !filepath.IsAbs(directory) || filepath.Clean(directory) != directory {
		return "", errors.New("OCI layout path is invalid")
	}
	if strings.ContainsAny(directory, "@?#\x00\r\n") {
		return "", errors.New("OCI layout path cannot be represented as a BuildKit context")
	}
	var layout installedLayoutDocument
	if err := readBoundedJSON(filepath.Join(directory, "oci-layout"), &layout); err != nil {
		return "", fmt.Errorf("read OCI layout version: %w", err)
	}
	if layout.ImageLayoutVersion != installedLayoutVersion {
		return "", errors.New("OCI layout version is unsupported")
	}
	var index oci.Index
	...
	if len(index.Manifests) != 1 {
		return "", errors.New("OCI layout must contain exactly one manifest")
	}
	...
	if descriptor.MediaType != installedManifestMedia ||
		...
		return "", errors.New("OCI layout manifest is invalid")
	}
	...
	if err != nil || !manifestInfo.Mode().IsRegular() || manifestInfo.Size() != descriptor.Size {
		return "", errors.New("OCI manifest is not a bounded regular file")
	}
	...
	if int64(len(manifest)) != descriptor.Size || sha256sum.DigestBytes(manifest) != descriptor.Digest {
		return "", errors.New("OCI manifest does not match its descriptor")
	}
