...
import { readFile, realpath as realpath3, stat as stat2 } from "node:fs/promises";
...
async function deriveLocalPackages(root, manager) {
  ...
  while (pending.length !== 0) {
    ...
    for (const dependency of localDependencyTargets(manifest)) {
      const target = await realpath3(resolve4(canonicalRoot, sourceRoot, dependency));
...
async function pnpmWorkspacePatterns(root) {
  let raw;
  try {
    raw = await readFile(resolve4(root, "pnpm-workspace.yaml"), "utf8");
  } catch (error) {
    if (error.code === "ENOENT")
      return [];
    throw error;
...
async function compilerTSConfigs(root, inputs) {
  ...
  while (pending.length !== 0) {
    ...
    if (errors2.length !== 0 || typeof document !== "object" || document === null || Array.isArray(document)) {
...
async function resolveTSConfigExtends(configPath, specifier) {
  ...
  const require2 = createRequire3(pathToFileURL(configPath));
...
async function main() {
  ...
  if (process.argv.length !== 7 || process.argv[2] === undefined || process.argv[3] === undefined || process.argv[4] === undefined || process.argv[5] === undefined || !isManager(process.argv[6])) {
    throw new Error("Program Compiler requires a Program root, canonical config path, exact Node version, output root, and Manager family");
  }
