...
type squashFSMetadataCursor struct {
	ctx      context.Context
...
func newSquashFSMetadataCursor(
	ctx context.Context,
	decoder *squashFSMetadataDecoder,
	region squashFSRegion,
	offset uint64,
	position uint16,
) (*squashFSMetadataCursor, error) {
	if decoder == nil || decoder.reader == nil || decoder.decoder == nil {
		return nil, &artifactInfrastructureError{
			cause: fmt.Errorf("SquashFS metadata decoder is not initialized"),
		}
	...
	if region.start > region.end || offset < region.start || offset >= region.end {
		return nil, &artifactContentError{
			cause: fmt.Errorf(
				"SquashFS metadata cursor offset %d is outside [%d, %d)",
				offset,
				region.start,
				region.end,
			),
	...
	block, err := decoder.readBlock(ctx, region, offset, 0)
	if err != nil {
		return nil, err
	}
...
func (cursor *squashFSMetadataCursor) readFull(destination []byte) error {
	...
	for len(destination) > 0 {
		if cursor.position == len(cursor.block) {
			block, err := cursor.decoder.readBlock(
				cursor.ctx,
				cursor.region,
				cursor.next,
				0,
