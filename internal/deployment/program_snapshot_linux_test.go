...
func TestProgramSnapshotOwnsImmutableUnnamedBytes(t *testing.T) {
	...
	if len(entries) != 1 || entries[0].Name() != "encoder-output" {
		t.Fatalf("snapshot created visible entries: %#v", entries)
	}
...
func TestProgramSnapshotUploadRevalidatesDescriptor(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if _, err := io.ReadAll(reader); err == nil || !strings.Contains(err.Error(), test.wantError) {
