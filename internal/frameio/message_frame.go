...
func ReadMessageFrame(r io.Reader) ([]byte, error) {
	return ReadMessageFrameBounded(r, MaxFrameBytes)
}

func ReadMessageFrameBounded(r io.Reader, maxBytes uint32) ([]byte, error) {
	var header [4]byte
	...
	if size > maxBytes {
		return nil, fmt.Errorf("frameio message frame length %d exceeds max %d", size, maxBytes)
	}
	body := make([]byte, size)
