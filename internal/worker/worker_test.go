...
type materializerFunc func(ctx context.Context, mount api.WorkerWorkspaceMount, client api.WorkerWorkspaceMaterializerControlClient) error

func (f materializerFunc) RunWorkspaceMount(ctx context.Context, mount api.WorkerWorkspaceMount, client api.WorkerWorkspaceMaterializerControlClient) error {
	return f(ctx, mount, client)
}
...
func TestRunOnceStartsWorkspaceMountWithoutRunLease(t *testing.T) {
	called := make(chan api.WorkerWorkspaceMount, 1)
	client := &fakeClient{
		mountClaim: api.WorkerWorkspaceMountClaimResponse{
	...
	runner := newTestRunner(t, client, executorFunc(func(context.Context, api.WorkerRunLeaseProvider, api.WorkerRun) api.WorkerReleaseResult {
		t.Fatal("executor should not run for mount-only work")
		return api.WorkerReleaseResult{}
	}))
	runner.materializer = materializerFunc(func(_ context.Context, mount api.WorkerWorkspaceMount, _ api.WorkerWorkspaceMaterializerControlClient) error {
		called <- mount
		return nil
	})
	worked, err := runner.RunOnce(context.Background())
	...
	case mount := <-called:
		if mount.ID != "mat-1" {
			t.Fatalf("mount id = %q", mount.ID)
		}
	case <-time.After(time.Second):
		t.Fatal("materializer was not called")
	}
	if client.mountClaims != 1 {
		t.Fatalf("mount claims = %d", client.mountClaims)
	}
}

func TestRunOnceClaimsWorkspaceMountsWhileAnotherIsActive(t *testing.T) {
	block := make(chan struct{})
	started := make(chan string, 2)
	client := &fakeClient{
		mountClaimQueue: []api.WorkerWorkspaceMountClaimResponse{
			{
				Mount: &api.WorkerWorkspaceMount{
					ID:                   "mat-1",
					OrgID:                "org-1",
					WorkspaceID:          "workspace-1",
					RuntimeInstanceToken: "reservation-token-1",
					FencingGeneration:    1,
					ExpiresAt:            time.Now().Add(time.Minute),
				},
			},
			{
				Mount: &api.WorkerWorkspaceMount{
					...
					RuntimeInstanceToken: "reservation-token-2",
					FencingGeneration:    1,
					ExpiresAt:            time.Now().Add(time.Minute),
				},
			},
	...
	case id := <-started:
		if id != "mat-2" {
			t.Fatalf("second mount id = %q", id)
		}
	case <-time.After(time.Second):
		t.Fatal("second materializer did not start")
	}
	if client.mountClaims != 2 {
		t.Fatalf("mount claims = %d, want 2", client.mountClaims)
	}
	close(block)
	runner.waitForMounts()
}

func TestRunWaitsForWorkspaceMountOnShutdown(t *testing.T) {
	...
	runner.materializer = materializerFunc(func(ctx context.Context, _ api.WorkerWorkspaceMount, _ api.WorkerWorkspaceMaterializerControlClient) error {
		started <- struct{}{}
		<-ctx.Done()
		<-release
		return nil
...
func (f *fakeClient) ClaimWorkspaceMount(context.Context, api.WorkerCapabilities) (api.WorkerWorkspaceMountClaimResponse, error) {
	f.mountClaims++
	if len(f.mountClaimQueue) > 0 {
		response := f.mountClaimQueue[0]
		f.mountClaimQueue = f.mountClaimQueue[1:]
		return response, nil
	}
	return f.mountClaim, nil
}

func (f *fakeClient) RenewWorkspaceMount(context.Context, api.WorkerWorkspaceMountRenewRequest) (api.WorkspaceMountResponse, error) {
	return api.WorkspaceMountResponse{State: "mounted"}, nil
}

func (f *fakeClient) MarkWorkspaceMountMounted(context.Context, api.WorkerWorkspaceMountMountedRequest) (api.WorkspaceMountResponse, error) {
	return api.WorkspaceMountResponse{State: "mounted"}, nil
}

func (f *fakeClient) CaptureWorkspaceMount(context.Context, api.WorkerWorkspaceMountCaptureRequest) (api.WorkerWorkspaceMountCaptureResponse, error) {
	return api.WorkerWorkspaceMountCaptureResponse{VersionID: "version-1"}, nil
}

func (f *fakeClient) StopWorkspaceMount(context.Context, api.WorkerWorkspaceMountStopRequest) (api.WorkspaceMountResponse, error) {
	f.mountStops++
	return api.WorkspaceMountResponse{State: "unmounted"}, nil
}

func (f *fakeClient) FailWorkspaceMount(context.Context, api.WorkerWorkspaceMountFailRequest) (api.WorkspaceMountResponse, error) {
	f.mountFailures++
	return api.WorkspaceMountResponse{State: "failed"}, nil
}

func (f *fakeClient) ClaimWorkspaceOperation(context.Context, api.WorkerWorkspaceOperationClaimRequest) (api.WorkerWorkspaceOperationClaimResponse, error) {
	f.operationClaims++
	return f.operationClaim, nil
}

func (f *fakeClient) StartWorkspaceOperation(context.Context, api.WorkerWorkspaceOperationStartRequest) (api.WorkspaceOperationResponse, error) {
	return api.WorkspaceOperationResponse{State: "running"}, nil
}

func (f *fakeClient) CompleteWorkspaceOperation(_ context.Context, request api.WorkerWorkspaceOperationCompleteRequest) (api.WorkspaceOperationResponse, error) {
