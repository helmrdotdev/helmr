...
func claimActorRunLeaseInTx(
	...
	if authority.actor.DispatchHoldID.Valid || authority.actor.ActiveTurnID.Valid || authority.actor.CurrentRunID != locators.RunID ||
...
func claimCheckpointRestoreRunLeaseInTx(
	...
	if locators.SessionID.Valid {
		...
		if authority.actor.DispatchHoldID.Valid || !locators.ActorRunGeneration.Valid ||
	...
	if sameWorkspaceParentResumeWait(authority.runWait) && !authority.runWait.ChildWriterGeneration.Valid {
		excluded, err := q.SameWorkspaceChildHasNoExecution(ctx, db.SameWorkspaceChildHasNoExecutionParams{
			ChildRunID: authority.runWait.ChildRunID, ParentRunID: authority.run.ID,
			WorkspaceID: authority.workspace.ID, BaseWorkspaceVersionID: authority.runWait.BaseWorkspaceVersionID,
		})
		if err != nil {
			return runLeaseClaimAuthority{}, err
		}
...
func claimRunLeasePhysicalInTx(
	...
	leaseSequence int64,
	locators db.GetRunLeaseClaimLocatorsRow,
	expectedBaseWorkspaceVersionID pgtype.UUID,
	...
	if err := validateClaimWorkspaceAuthority(authority, expectedBaseWorkspaceVersionID); err != nil {
		return runLeaseClaimAuthority{}, err
	}
...
func markRunLeaseStartingInTx(
	...
	if authority.runLease.Status != db.RunLeaseStatusAssigned {
		return authority, nil
	}
...
func validateClaimWorkspaceAuthority(
	...
	if authority.workspaceLease.BaseWorkspaceVersionID != expectedBaseWorkspaceVersionID {
		return errStaleRunLeaseClaim
	}
	return nil
}
...
func validateCheckpointRestoreWait(
	...
	if sameWorkspaceParentResumeWait(wait) {
		if !wait.ChildRunID.Valid ||
			...
			wait.ParentWriterGeneration.Int64 >= wait.ResumeWriterGeneration.Int64 ||
		...
		if wait.ChildWriterGeneration.Valid {
			if wait.ParentWriterGeneration.Int64 >= wait.ChildWriterGeneration.Int64 || wait.ChildWriterGeneration.Int64 >= wait.ResumeWriterGeneration.Int64 {
				return errStaleRunLeaseClaim
			}
		} else if wait.ConditionStatus != db.WaitStatusFailed && wait.ConditionStatus != db.WaitStatusCancelled {
			return errStaleRunLeaseClaim
		}
		switch wait.ConditionStatus {
		case db.WaitStatusCompleted:
		case db.WaitStatusFailed, db.WaitStatusCancelled:
...
func validateCheckpointRestore(authority runLeaseClaimAuthority) error {
	...
	if checkpoint.Status != db.RunCheckpointStatusReady ||
	...
	if sameWorkspaceParentResumeWait(authority.runWait) {
		if checkpoint.PrivateWorkspaceVersionID != authority.runWait.BaseWorkspaceVersionID ||
			authority.workspaceLease.BaseWorkspaceVersionID != authority.runWait.ResumeWorkspaceVersionID {
			return errStaleRunLeaseClaim
		}
	} else if checkpoint.PrivateWorkspaceVersionID != authority.workspaceLease.BaseWorkspaceVersionID {
		return errStaleRunLeaseClaim
	}
...
func validateCheckpointSource(authority runLeaseClaimAuthority) error {
	...
	if !runtimeHasExclusionProof(sourceRuntime) ||
		...
		(sourceWorkspaceLease.Status != db.WorkspaceLeaseStatusReleased &&
			sourceWorkspaceLease.Status != db.WorkspaceLeaseStatusFenced) ||
		sourceWorkspaceLease.WorkspaceID != authority.checkpoint.WorkspaceID ||
		sourceWorkspaceLease.BaseWorkspaceVersionID != authority.checkpoint.BaseWorkspaceVersionID ||
