...
func TestSharedComputerCheckpointFailureStopsOwnerAndChildPostgres(t *testing.T) {
	...
	dbtest.MustExec(t, ctx, f.pool, `UPDATE run_leases SET status='checkpointing',finalization_operation_id=NULL,finalization_started_at=NULL,finalization_request_fingerprint=NULL,finalization_root=NULL WHERE id=$1`, childLease)
