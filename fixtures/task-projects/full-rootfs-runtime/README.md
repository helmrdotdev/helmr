...
- `contract` verifies Debian slim task execution after `apt-get install -y curl`, `/usr/bin` visibility, image env propagation, image `PATH` precedence over the runtime default, image `WorkingDir`, and GitHub checkout workspace precedence over image files.
...
- `approval` verifies the `ctx.wait.approval` request/response relay.
- `impl` is the source/image fixture. Its image copies only `source.file("package.json")` and `source.file("bun.lockb")`, runs an install-like image layer with a cache mount, and uses the GitHub checkout mounted at `/workspace`. The gated E2E harness uses it to verify fresh build, same-lockfile cache hit, code-only source changes, lockfile image-key changes, and 10-way singleflight.
- `alpine-starts` and `distroless-starts` verify that the runtime bundle starts the adapter in Alpine and shell-less/distroless-style rootfs images.
