...
export const contract = task({
  ...
  run: async (_payload, ctx) => {
    ...
    await assertVisibleFile("/tmp/task/rootfs-cwd-write.txt", "rootfs cwd\n")
    await assertVisibleFile("/tmp/rootfs-tmp-write.txt", "rootfs tmp\n")
    await assertVisibleFile("/var/log/rootfs-log-write.txt", "rootfs log\n")
    await assertVisibleFile(`${process.env["HOME"]}/.cache/rootfs-cache-write.txt`, "rootfs cache\n")
