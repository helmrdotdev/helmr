...
export function useRunRowNavigation(run: () => { id: string; project_id: string; environment_id: string }) {
  const navigate = useNavigate();
  const openRun = (event: MouseEvent) => {
    if (event.target instanceof Element && event.target.closest(INTERACTIVE_TARGETS)) return;
    const current = run();
    navigate(runHref(current.id, current.project_id, current.environment_id));
  };
