...
export function Waitpoints() {
  ...
  const waitpointRuns = createMemo(() => (runs.data?.runs ?? []).filter((run) => run.pending_waitpoint));
  ...
  return (
    <section class={ui.page}>
      ...
      <Show when={runs.isError}>
        <p class={ui.error} role="alert">{waitpointsErrorMessage(runs.error)}</p>
      </Show>

      <Show when={!runs.isPending} fallback={<p class={ui.muted}>Loading waitpoints...</p>}>
        <Show
          when={waitpointRuns().length > 0}
          fallback={
                  ...
                  {(run) => {
                    ...
                    return (
                      <tr class={ui.clickableTableRow} {...rowNavigation}>
                        ...
                        <td>
                          <span class={ui.muted}>
                            {run.pending_waitpoint?.kind ?? "—"}
                          </span>
                        </td>
                        <td>
                          <span class={ui.muted}>
                            {formatRelative(run.pending_waitpoint?.created_at ?? run.updated_at)}
                          </span>
                        </td>
                      </tr>
                    );
