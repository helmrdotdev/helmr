...
- `IDText` — every ID, digest, and key: monospace, shows the tail (`…` + last 12 characters) so IDs that share a time-based prefix stay distinguishable, full value in `title`, click copies. Pass `full` in detail rails, `href` when the ID links to its page (renders the link plus a separate copy button; never nest `IDText` inside a link), and `fallback` for the empty case (default `—`). `formatID` in `ui/id.ts` is the shared truncation.
