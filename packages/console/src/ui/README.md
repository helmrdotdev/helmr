...
- Buttons: `ui.button` for the primary action, `ui.secondaryButton` for the rest, `ui.dangerOutlineButton` for destructive header and modal actions, `ui.ghostButton` / `ui.ghostDangerButton` for quiet inline actions in Overview rows and section headers.
...
| Overview "Needs you" | Short form (`IDText mode="link"` to the detail page). | Inline actions stay as quiet `ui.ghostButton` / `ui.ghostDangerButton` controls with no fill and a hover background; the confirm or complete modal shows what is being acted on (token id, tags, metadata when available). Cancel of an in-progress Run on the Overview is a ghost button too. |
