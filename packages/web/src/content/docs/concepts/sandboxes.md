...
Helmr manages the runtime substrate around the image, including `/proc`, `/dev`, `/dev/shm`, DNS resolver files, hostname setup, logs, session streams, waits, and secret injection. See [Runtime environment](/docs/concepts/runtime-environment/) for the task-visible runtime contract.
