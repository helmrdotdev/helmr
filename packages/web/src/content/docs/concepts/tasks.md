...
The task context provides `ctx.log`, `ctx.emit`, `ctx.signal`, `ctx.run.id`, and `ctx.wait` for human and delay waitpoints. The return value becomes run output when the task succeeds.
