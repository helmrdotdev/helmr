...
| `/readyz` fails | Database URL secret is wrong, RDS is unavailable, or migrations have not run successfully. |
...
| Run stays queued | No active workers, worker pool desired capacity is zero, worker registration failed, or worker cannot reach the control plane. |
...
For control task failures, inspect ECS service events and CloudWatch logs for the control service. For worker failures, use SSM to inspect systemd journals on the worker host.
