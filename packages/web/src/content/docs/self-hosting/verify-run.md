...
After the control service is ready and at least one worker is active, verify the environment with a small run. A default `quickstart` stack must enable NAT and worker capacity before this step.
...
- A worker is active and can claim work.
