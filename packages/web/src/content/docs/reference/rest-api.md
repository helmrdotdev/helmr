...
`POST /api/schedules` creates an imperative schedule. The request body uses `deduplication_key`, `external_id`, `task`, `cron`, `timezone`, `workspace`, `secret_bindings`, and optional run `options`. Schedule requests do not accept arbitrary payload; scheduled runs receive Helmr-generated schedule metadata. Declarative schedules are synchronized from deployments and return `400 Bad Request` for imperative edit, activate, deactivate, or delete routes.
